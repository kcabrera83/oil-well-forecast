...
def test_models():
    results = {}
    results['production_forecaster'] = evaluate_model(os.path.join(MODEL_DIR, 'production_forecaster.pkl'), X, y)
    results['eur_estimator'] = evaluate_model(os.path.join(MODEL_DIR, 'eur_estimator.pkl'), X, y)
    return results
